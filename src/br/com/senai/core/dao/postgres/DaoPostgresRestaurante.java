...
public class DaoPostgresRestaurante implements DaoRestaurante {
	...
	private final String UPDATE = "UPDATE restaurantes SET nome = ?, descricao = ?, "
								...
								+ "complemento = ?, id_categoria = ? "
								+ "WHERE id = ?";
	
	private final String DELETE = "DELETE FROM restaurantes WHERE id = ?";
	
	private final String SELECT_BY_ID = "SELECT r.id id_restaurante, r.nome nome_restaurante, "
										+ "r.descricao, r.cidade, r.logradouro, r.bairro, "
										+ "r.complemento, c.id id_categoria, c.nome nome_categoria "
